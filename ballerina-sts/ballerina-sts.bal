...
configurable int TOKEN_VALIDITY_PERIOD = 3600; // in seconds

// Credentials of the mock authorization server.
string USERNAME = "author";
string PASSWORD = "author";
...
string[] usernames = ["admin", "author", "buyer"];
string[] passwords = ["admin", "author", "buyer"];
string[] accessTokenStore = ["56ede317-4511-44b4-8579-a08f094ee8c5", "YXV0aG9yOmF1dGhvcjphdXRob3I=", "YWRtaW46YWRtaW46YWRtaW4=", "YnV5ZXI6YnV5ZXI6YnV5ZXI="];
    ...
    resource function post introspect(http:Request req) returns json|http:Unauthorized|http:BadRequest {
        ...
        if authorizationHeader is string {
            ...
            if payload is string {
                ...
                json|error introspectionRes = prepareIntrospectionResponse(token, tokenTypeHint);
                if introspectionRes is error {
                    string description = "Invalid client with invalid token or no scopes";
                    return createInvalidClient(description);
                }
...
function prepareTokenResponse(string grantType, string username, string password, string refreshToken, string scopes) returns json|http:Unauthorized|http:BadRequest|http:InternalServerError {
    if grantType == GRANT_TYPE_CLIENT_CREDENTIALS {
        ...
        if usernames.indexOf(username) !is () && passwords.indexOf(password) !is () {
            string accessTokenString = scopes != "" ? string `${username}:${password}:${scopes}` : string `${username}:${password}`;
            string newAccessToken = accessTokenString.toBytes().toBase64();
            addToAccessTokenStore(newAccessToken);
...
function prepareIntrospectionResponse(string accessToken, string tokenTypeHint) returns json|error? {
    foreach string token in accessTokenStore {
        if token == accessToken {
            // Extract scope
            byte[] tokenBytes = check arr:fromBase64(token);
            string decodedToken = check str:fromBytes(tokenBytes);
            string[] params = regex:split(decodedToken, ":");
            string scopes = params[2];
